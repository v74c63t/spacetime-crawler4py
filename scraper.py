...
def report_info(url, resp):
    # text was tokenized multiple times, modified tokenizer to only tokenize once
    unfiltered_words = tokenize.tokenize(resp.raw_response.content, url)

    global word_freq
    word_freq = tokenizer.filterTokens(unfiltered_words, word_freq)
    global most_common_words
    most_common_words = sorted(word_freq.items(), key=lambda x: x[1], reverse=True)[0:50]
    # maybe sort this after crawling every page ?

    # use tokenizer here to look at content for report info?
    global largest_pg # check if correct
    if len(unfiltered_words) > largest_pg[1]: largest_pg = [url, len(unfiltered_words)]
